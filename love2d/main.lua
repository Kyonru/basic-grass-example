...
local canvas
local grassShader, floorShader, capsuleShader
local grassMesh, floorMesh, capsuleMesh
local textures = {}
local characters = {}
...
local function make_view_matrix()
    return {
        CAM_X[1], CAM_Y[1], CAM_Z[1], 0.0,
        CAM_X[2], CAM_Y[2], CAM_Z[2], 0.0,
        CAM_X[3], CAM_Y[3], CAM_Z[3], 0.0,
        -dot3(CAM_X, CAM_POS), -dot3(CAM_Y, CAM_POS), -dot3(CAM_Z, CAM_POS), 1.0,
    }
end
...
local function make_noise_texture(size, periods, seed)
    ...
    data:mapPixel(function(x, y)
        ...
        local b = (y / size) * tau
        local v = lm.noise(
            seed + radius * math.cos(a), radius * math.sin(a),
            seed * 0.61803 + radius * math.cos(b), radius * math.sin(b))
        return v, v, v, 1.0
    end)
...
local function scatter_grass(mesh)
    local offsets = {}
    ...
    local instances = lg.newMesh({ { "InstanceOffset", "float", 3 } }, offsets, nil, "static")
...
local function make_capsule_mesh(radius, height, segments, rings)
    ...
    for _, row in ipairs(rows) do
        ...
        for s = 0, segments do
            ...
            verts[#verts + 1] = {
                nx * radius, ny * radius + row.y_off, nz * radius,
                nx, ny, nz,
            }
        end
    end
...
local function spawn_characters()
    ...
    characters = {
        { pos = { -2.22791, 0.0, 5.80089 }, speed = 2.0, accel = 5.0, size = 0.86,
            mesh_scale = { 2.0, 1.0, 2.0 }, mesh_y = 0.75 },
        { pos = { -5.84467, 0.0, 5.51433 }, speed = 2.5, accel = 5.0, size = 0.53,
            mesh_scale = { 1.0, 1.0, 1.0 }, mesh_y = 0.75 },
        { pos = { -2.62366, 0.0, 8.07181 }, speed = 3.0, accel = 5.0, size = 0.38,
            mesh_scale = { 0.5, 0.5, 0.5 }, mesh_y = 0.4 },
    }
...
local function send_static_uniforms()
    ...
    floorShader:send("u_ndotl", LIGHT_DIR[2]) -- floor normal is straight up
...
function love.draw()
    ...
    lg.print("Space: stepped wind (" .. tostring(quantised) .. ")  |  C: displacement ("
        .. tostring(displacement_enabled) .. ")  |  N: wind debug  |  R: rescatter  |  Esc: quit", 12, 12)
