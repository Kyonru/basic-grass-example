...
local function draw_grass()
	...
	for _, blade in ipairs(blades) do
		...
		local n1 = love.math.noise(
			(blade.x + HALF_FIELD) * WIND_SCALE + wind_noise_base,
			(blade.z + HALF_FIELD) * WIND_SCALE + t * 0.25
		)
		local n2 = love.math.noise(
			(blade.x + HALF_FIELD) * (WIND_SCALE * 0.8) + t * 0.22,
...
function love.draw()
	...
	love.graphics.print(
		"WASD/Arrows: Pan  |  Mouse Wheel: Zoom  |  Space: Toggle stepped wind  |  R: Regenerate  |  F1: Debug",
		12,
		12
	)
	...
	if show_debug then
		...
		love.graphics.print("fps: " .. love.timer.getFPS(), 12, 124)
	end
end
